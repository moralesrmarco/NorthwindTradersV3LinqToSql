...
namespace NorthwindTradersV3LinqToSql
{
    public partial class FrmTableroControlAltaDireccion : Form
    {
        ...
        private void LlenarCmbVentasVendedorAnio()
        {
            ...
            try
            {
                using (var context = new NorthwindTradersDataContext())
                {
                    ...
                    .ToList();
                    cmbVentasMensualesDelAnio.DataSource = years;
                }
            }
            ...
            cmbVentasMensualesDelAnio.SelectedIndex = 0;
            cmbVentasMensualesDelAnio.SelectedIndexChanged += cmbVentasMensualesDelAnio_SelectedIndexChanged;
        }
        ...
        {
            CargarVentasMensuales(Convert.ToInt32(cmbVentasMensualesDelAnio.SelectedItem), (SeriesChartType)CmbTipoGrafica1.SelectedItem);
        }
        ...
        private void CargarVentasMensuales(int year, SeriesChartType tipoGrafica)
        {
            ...
            if (datos != null && datos.Rows.Count > 0)
            {
                ...
                foreach (var punto in datos.AsEnumerable())
                {
                    string nombreMes = punto.Field<string>("NombreMes");
                    int index = serie.Points.AddXY(nombreMes, punto.Field<decimal>("Total"));
                    DataPoint dataPoint = serie.Points[index];
                    dataPoint.Label = $"${punto.Field<decimal>("Total"):#,##0.00}";
